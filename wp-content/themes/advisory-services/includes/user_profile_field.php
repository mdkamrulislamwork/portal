...
function extra_user_profile_fields( $user ) { 
    ...
    if (in_array('viewer', $user->roles) && array_intersect(['administrator', 'advisor'], $admin->roles)) {
        ...
        echo '<table class="form-table extraProfileInformation">';
            ...
            echo '<tr>';
            echo '<tr>';
                $specialCMAUser = esc_attr(get_the_author_meta( 'specialcmauser', $user->ID ));
                if ($specialCMAUser) { $specialsfiauserYes = ' checked'; $specialsfiauserNo = ''; } 
                else { $specialsfiauserYes = ''; $specialsfiauserNo = ' checked'; }
                echo '<th><label for="spuser">Special Cloud Maturity user</label></th>';
                echo '<td>';
                echo '<label for="specialcmauser-yes"><input type="radio" name="specialcmauser" id="specialcmauser-yes" value="1"'. $specialsfiauserYes .'> Yes</label>';
                echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
                echo '<label for="specialcmauser-no"><input type="radio" name="specialcmauser" id="specialcmauser-no" value="0"'. $specialsfiauserNo .'> No</label>';
                echo '</td>';
            echo '<tr>';
            echo '<tr>';
                $specialFacilityUser = esc_attr(get_the_author_meta( 'specialfacilityuser', $user->ID ));
                if ($specialFacilityUser) { $specialfacilityuserYes = ' checked'; $specialfacilityuserNo = ''; } 
                else { $specialfacilityuserYes = ''; $specialfacilityuserNo = ' checked'; }
                echo '<th><label for="spuser">Special Facility user</label></th>';
                echo '<td>';
                echo '<label for="specialfacilityuser-yes"><input type="radio" name="specialfacilityuser" id="specialfacilityuser-yes" value="1"'. $specialfacilityuserYes .'> Yes</label>';
                echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
                echo '<label for="specialfacilityuser-no"><input type="radio" name="specialfacilityuser" id="specialfacilityuser-no" value="0"'. $specialfacilityuserNo .'> No</label>';
                echo '</td>';
            echo '<tr>';
            echo '<tr>';
                $specialCybersecurityUser = esc_attr(get_the_author_meta( 'specialcybersecurityuser', $user->ID ));
                if ($specialCybersecurityUser) { $specialCybersecurityUserYes = ' checked'; $specialCybersecurityUserNo = ''; } 
                else { $specialCybersecurityUserYes = ''; $specialCybersecurityUserNo = ' checked'; }
                echo '<th><label for="spuser">Special Cybersecurity user</label></th>';
                echo '<td>';
                echo '<label for="specialcybersecurityuser-yes"><input type="radio" name="specialcybersecurityuser" id="specialcybersecurityuser-yes" value="1"'. $specialCybersecurityUserYes .'> Yes</label>';
                echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
                echo '<label for="specialcybersecurityuser-no"><input type="radio" name="specialcybersecurityuser" id="specialcybersecurityuser-no" value="0"'. $specialCybersecurityUserNo .'> No</label>';
                echo '</td>';
            echo '<tr>';
        echo '</table>'; 
    } ?>
...
function save_extra_user_profile_fields( $user_id ) {
    ...
    if (isset($_POST['spuser'])) {
        ...
        update_user_meta( $user_id, 'specialcmauser', $_POST['specialcmauser']);
        update_user_meta( $user_id, 'specialfacilityuser', $_POST['specialfacilityuser']);
        update_user_meta( $user_id, 'specialcybersecurityuser', $_POST['specialcybersecurityuser']);
    }
}