...
// MISSING SKILLS
$missingSkills = advisory_form_default_values($sfia_id, 'missing_skills');
$missingSkillsDate = !empty($missingSkills['time']) ? date( get_option('date_format'), $missingSkills['time'] ) : false;
?>
                    ...
                    <div class="sfia-header">
                        <div class="row">
                            ...
                            <div class="col-sm-12 sfiaAddEditBtnContainer">
                                ...
                                <?php if (!$select_attr) {
                                    $activeOrArchivedItem = advisory_sfia_get_active_or_archive_post_id($user_company_id);
                                    echo '<button type="button" class="btn btn-lg btn-primary addBtn add_sfia_user"><span class="fa fa-user-plus"></span> Add Role</button><br>';
                                    if ( $activeOrArchivedItem ) {
                                        echo '<a href="'.site_url('sfia-missing-skills/').'" class="btn btn-lg btn-primary" style="background:#a349a4"><span class="fa fa-user-plus"></span> Add Missing Skill</a>';
                                        echo '<a href="'.site_url('sfia-partial-skills-fit/').'" class="btn btn-lg btn-primary" style="background:#bd6817"><span class="fa fa-user-plus"></span> Partial Skills Fit</a>';
                                    } else {
                                        echo '<a href="javascript:;" class="btn btn-lg btn-primary sfiaMSNoItem" style="background:#a349a4"><span class="fa fa-user-plus"></span> Add Missing Skill</a>';
                                        echo '<a href="javascript:;" class="btn btn-lg btn-primary sfiaPSFNoItem" style="background:#bd6817"><span class="fa fa-user-plus"></span> Partial Skills Fit</a>';
                                    }
                                } ?>
                            </div>
                            ...
                            <?php if ($query->have_posts()) {
                                while ($query->have_posts()) {
                                    ...
                                    echo '<tr>
                                        ...
                                        <td>'. date( 'n/d/y', strtotime(get_the_date()) ) .'</td>
                                        <td class="text-right">
                ...
                <div class="card-body">
                    ...
                    <?php $query = new WP_Query(['post_type' => 'sfia', 'post_status' => ['publish','archived'], 'meta_query' => array(array('key' => 'assigned_company', 'value' => $user_company_id))]);
                        if ($query->have_posts()) {
                            ...
                            if ($scorecardHTML) { 
                                if ($missingSkillsDate) $scorecardHTML .= '<option value="#" team_id="#" team_name="#" management="#" team="#" type="missing_skills">SFIA Assessment Missing Skills - '.$missingSkillsDate.'</option>';
                                echo '<select name="" id="" class="ajax-sfia-scorecard-select bold">'.$scorecardHTML.'</select>'; 
                            }
...
<div class="modal fade modal-inverse" id="sfiaMSNoItem">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                ...
                <h4 class="modal-title">Missing Skills</h4>
            </div>
            <div class="modal-body p-0"><h3 class="color-red text-center" style="margin: 20px;">SFIA Module Not Active</h3></div>
            <div class="modal-footer">
                <button type="button" class="btn btn-sm btn-default" data-dismiss="modal">Close</button>
            </div>
        </div><!-- /.modal-content -->
...
<div class="modal fade modal-inverse" id="sfiaPSFNoItem">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                ...
                <h4 class="modal-title">Partial Skills</h4>
            </div>
            <div class="modal-body p-0"><h3 class="color-red text-center" style="margin: 20px;">SFIA Module Not Active</h3></div>
            <div class="modal-footer">
                <button type="button" class="btn btn-sm btn-default" data-dismiss="modal">Close</button>
...
(function($) {
    ...
    $(document).on('click', '.sfiaMSNoItem', function(e) {
        e.preventDefault();
        $('#sfiaMSNoItem').modal('show');
    })
    $(document).on('click', '.sfiaPSFNoItem', function(e) {
        e.preventDefault();
        $('#sfiaPSFNoItem').modal('show');
    })
    ...
    $(document).on('click', '.SFIAInfoBtn', function(e) {
        ...
        var info_text = $(this).attr('title');
        var activeSkill = $(this).parents('.skillContainer').find('.skills').val();
        var modal = $('#SFIALevelInfoModal');
        modal.find('.modal-title').text(info_text);
