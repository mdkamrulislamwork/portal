...
if ($postID == 'sfia_missing_skills') {
    $filename   = "SFIA Missing Skills";
    $dompdf     = new Dompdf();
    $company    = advisory_get_user_company();
    $post_id    = advisory_sfia_get_active_post_id($company->term_id);
    $opts       = get_post_meta($post_id, 'form_opts', true);
    $default    = advisory_form_default_values($post_id, 'missing_skills');
    if ( !empty($default) ) { 
        unset($default['time']); 
        $notes      = $default['notes'];        unset($default['notes']); 
        $summaries    = $default['summaries'];  unset($default['summaries']); 
        $recommendations  = $default['recommendations'];    unset($default['recommendations']); 
    }

    $skills     = advisory_sfiams_pdf_skills($opts, $default);
    $logo   = '<img src="'. IMAGE_DIR_URL .'sfia/pdf/sfia.jpg" alt="Logo">';
    // echo '<br><pre>'. print_r($summaries, true) .'</pre>'; exit();
    // PDF DATA
    $html = '';
    ...
    $html .= '<style>';
        ...
        // SKILLS STYLES
        $html .= '.skillsTitle {margin:15px 0 2px 0;font-size:15px;}';
        $html .= ' table.skills {border-top: 3px solid #000;border-bottom: 2px solid #000;}';
        ...
        $html .= ' table.summary tr th.left, table.summary tr td.left{border-right: 2px solid #000;padding-right:10px;}';
        $html .= ' table.summary tr th.right, table.summary tr td.right{padding-left:10px;}';
        $html .= ' table.summary tr td p.title{font-weight:bold}';
            ...
            $html .= '<td>';
                $html .= '<table class="table m-0 no-border headerTop">';
                    $html .= '<tr>';
                        ...
                        $html .= '<td class="customerName">'.$company->name.'</td>';
                    $html .= '</tr>';
                $html .= '</table>';
            $html .= '</td>';
        $html .= '</tr>';
    $html .= '</table>';
    // SKILLS
    if ( $notes ) $html .= '<p style="width:700px">'.$notes.'</p>';
    if (!empty($skills)) {
        $skillCount = !empty($skills) ? count($skills) : 0;
        $loop = $skillCount ? ceil($skillCount/2) : 0;
        $html .= '<p class="skillsTitle">Missing Skills</p>';
            ...
            $html .= '<tr>';
                $html .= '<td class="skillsContainer skillsLeftContainer">';
                    $html .= '<table class="table m-0 no-border skillItems skillsLeft">';
                        $html .= '<tr>';
                            ...
                            $html .= '<th class="text-left">Sub Category</th>';
                            $html .= '<th class="text-left">Skill</th>';
                            $html .= '<th class="text-center code">Code</th>';
                            $html .= '<th class="text-center target">Target</th>';
                        $html .= '</tr>';
                        if ($loop) {
                            for ($i=0; $i < $skillCount; $i+=2) { 
                            // for ($i=0; $i < $loop; $i++) { 
                                $html .= '<tr>';
                                    $html .= '<td class="text-left '.$skills[$i]['class'].'"> '.$skills[$i]['cat'].' </td>';
                                    $html .= '<td class="text-left '.$skills[$i]['class'].'"> '.$skills[$i]['subcat'].' </td>';
                                    $html .= '<td class="text-left '.$skills[$i]['class'].'"> '.$skills[$i]['name'].' </td>';
                                    $html .= '<td class="text-center '.$skills[$i]['class'].'">'. $skills[$i]['code'] .' </td>';
                                    $html .= '<td class="text-center bg-deepgreen">Level '.$skills[$i]['target'].' </td>';
                                $html .= '</tr>';
                            }
                        }
                    $html .= '</table>';
                ...
                $html .= '<td class="skillsContainer skillsRightContainer">';
                    $html .= '<table class="table m-0 no-border skillItems skillsRight">';
                        ...
                        $html .= '</tr>';
                        if ($loop) {
                            for ($j=1; $j < $skillCount; $j+=2) { 
                            // for ($j=$i; $j < $skillCount; $j++) { 
                                $html .= '<tr>';
                                    $html .= '<td class="text-left '.$skills[$j]['class'].'"> '.$skills[$j]['cat'].' </td>';
                                    $html .= '<td class="text-left '.$skills[$j]['class'].'"> '.$skills[$j]['subcat'].' </td>';
                                    $html .= '<td class="text-left '.$skills[$j]['class'].'"> '.$skills[$j]['name'].' </td>';
                                    $html .= '<td class="text-center '.$skills[$j]['class'].'">'. $skills[$j]['code'] .' </td>';
                                    $html .= '<td class="text-center bold bg-deepgreen">Level '.$skills[$j]['target'].' </td>';
                                $html .= '</tr>';
                            }
                        }
                    $html .= '</table>';
    ...
    // SUMMARY
    // echo '<br><pre>'.print_r([$summaries, $recommendations], true).'</pre>'; exit(); 
    if ( !empty($summaries[0]) && !empty($recommendations[0]) ) {
        $loop = count($summaries) > count($recommendations) ? count($summaries) : count($recommendations);
        for ($i=0; $i < $loop; $i++) { 
            $html .= '<table class="table m-0 no-border summary">';
                $html .= '<tr>';
                    $html .= '<td class="left half">'.$summaries[$i] ?? null .'</td>';
                    $html .= '<td class="right half">'.$recommendations[$i] ?? null .'</td>';
                $html .= '</tr>';
            $html .= '</table>';
        }
    ...
    $post_id    = advisory_sfia_get_active_post_id($company->term_id);
    $opts       = get_post_meta($post_id, 'form_opts', true);
    $default    = advisory_form_default_values($post_id, 'partial_skills_fit');
    if ( !empty($default) ) { 
        unset($default['time']); 
        $notes      = $default['notes'];        unset($default['notes']); 
        $summaries    = $default['summaries'];  unset($default['summaries']); 
    ...
    $skills     = advisory_sfiapsf_pdf_skills($opts, $default);
    $logo   = '<img src="'. IMAGE_DIR_URL .'sfia/pdf/sfia.jpg" alt="Logo">';
    // echo '<br><pre>'. print_r($skills, true) .'</pre>'; exit();
    // PDF DATA
    $html = '';
    ...
    if (!empty($skills)) {
        $skillCount = !empty($skills) ? count($skills) : 0;
        $loop = $skillCount ? ceil($skillCount/2) : 0;
        $html .= '<p class="skillsTitle">Partial Skills</p>';
        $html .= '<table class="table m-0 no-border skills">';
            $html .= '<tr>';
                $html .= '<td class="skillsContainer skillsLeftContainer">';
                    $html .= '<table class="table m-0 no-border skillItems skillsLeft">';
                        ...
                        if ($loop) {
                            for ($i=0; $i < $skillCount; $i+=2) { 
                                $evaluationLevel = $skills[$i]['evaluation'] ? 'Level '.$skills[$i]['evaluation'] : 'Missing';
                                $evalCls = advisory_get_sfia_access_level_container_class($skills[$i]['evaluation'], $skills[$i]['target']);
                                $html .= '<tr>';
                                    $html .= '<td class="text-left '.$skills[$i]['class'].'"> '.$skills[$i]['cat'].' </td>';
                                    $html .= '<td class="text-left '.$skills[$i]['class'].'"> '.$skills[$i]['subcat'].' </td>';
                                    $html .= '<td class="text-left '.$skills[$i]['class'].'"> '.$skills[$i]['name'].' </td>';
                                    $html .= '<td class="text-center '.$skills[$i]['class'].'">'. $skills[$i]['code'] .' </td>';
                                    $html .= '<td class="text-center bg-deepgreen">Level '.$skills[$i]['target'].' </td>';
                                    $html .= '<td class="text-center '.$evalCls.'">'.$evaluationLevel.' </td>';
                                $html .= '</tr>';
                            }
                        }
                    $html .= '</table>';
                ...
                $html .= '<td class="skillsContainer skillsRightContainer">';
                    $html .= '<table class="table m-0 no-border skillItems skillsRight">';
                        ...
                        if ($loop) {
                            for ($j=1; $j < $skillCount; $j+=2) {
                                $evaluationLevel = $skills[$j]['evaluation'] ? 'Level '.$skills[$j]['evaluation'] : 'Missing';
                                $evalCls = advisory_get_sfia_access_level_container_class($skills[$j]['evaluation'], $skills[$j]['target']);
                                $html .= '<tr>';
                                    $html .= '<td class="text-left '.$skills[$j]['class'].'"> '.$skills[$j]['cat'].' </td>';
                                    $html .= '<td class="text-left '.$skills[$j]['class'].'"> '.$skills[$j]['subcat'].' </td>';
                                    $html .= '<td class="text-left '.$skills[$j]['class'].'"> '.$skills[$j]['name'].' </td>';
    ...
    if ($postType == 'csa') {
        ...
        if ($data) {
            $html .= '<style>';
                ...
                $html .= 'table{width: 100%; margin-bottom: 1.3rem !important;}';
                $html .= 'ul {padding: 0; margin: 0;padding-left: 15px;}';
                $html .= '.bold{font-weight:700 !important;}';
                $html .= '.title{font-size:18px !important;text-transform: uppercase;}';
                $html .= '.content{font-size:13px !important;}';
                $html .= '.heading{font-size:17px !important;color: #fff;padding: 0 !important;}';
                $html .= '.subhead{font-size: 16px !important;color: #fff;}';
                $html .= '.gray{background-color:#585858;color: #fff;}';
                $html .= '.black{background-color:#000;color:#fff;}';
                $html .= '.blue{background-color: #001f5f;color:#fff;}';
                ...
                $html .= '.table td.intro {padding: 5px 10px 0px 10px !important;}';
                $html .= '.table td.intro p {margin-bottom:8px;}';
                $html .= '.table.rtoTable {margin-bottom:0!important;}';
                $html .= '.table.rtoTable td.fixedbg {border:none!important;padding:0!important;height:32px;width: 50px;}';
                $html .= 'td.nopadding {padding: 0 !important;}';
                ...
                $html .= '.table-bordered td.fixedbg{}';
                $html .= '.table-bordered td {border: 1px solid #6e7275 !important;}';
                $html .= '.table-bordered td.noborder {border: 0px !important;}';
                $html .= '.logo {height: 90px; width: auto}';
                $html .= '.small {font-size: 13px !important;}';
                ...
                $html .= ' .avoidBreak {page-break-inside: avoid !important; margin: 4px 0 4px 0; }';
            $html .= '</style>';
            ...
            foreach ($data as $key => $area) {
                ...
                if ($areaID != $department) continue;
                    ...
                    $html .= '<div class="tableContainer">';
                        $html .= '<table class="table table-bordered">';
                            $html .= '<tr>';
                                $html .= '<td class="subhead black align-middle" colspan="3" rowspan="2">'. $title .'</span></td>';
                                $html .= '<td class="heading black center align-middle bold" rowspan="2">'. $deptName .'</td>';
                                $html .= '<td class="center light-gray" style="width:50px">Staff</td>';
                                $html .= '<td class="black center" style="width:130px">RTO</td>';
                            $html .= '</tr>';
                            $html .= '<tr>';
                                $html .= '<td class="center light-gray">'. $staff .'</td>';
                                $html .= '<td class="nopadding">';
                                    $html .= '<table class="table table-bordered rtoTable">';
                                        $html .= '<tr>';
                                            $html .= '<td class="fixedbg color-four">&nbsp;</td>';
                            ...
                            $html .= '';
                            $html .= '<tr>';
                                $html .= '<td class="small intro" colspan="5" rowspan="3">'. $intro .'</td>';
                                $html .= '<td class="nopadding">';
                                    $html .= '<table class="table table-bordered rtoTable">';
                                        $html .= '<tr>';
                                            $html .= '<td class="fixedbg color-five">&nbsp;</td>';
                                            $html .= '<td class="noborder">7-days</td>';
                                        $html .= '</tr>';
                                    $html .= '</table>';
                                $html .= '</td>';
                            $html .= '</tr>';
                            $html .= '<tr>';
                                $html .= '<td class="nopadding">';
                                    $html .= '<table class="table table-bordered rtoTable">';
                                        $html .= '<tr>';
                                            $html .= '<td class="fixedbg color-three">&nbsp;</td>';
                                            $html .= '<td class="noborder">3-days &nbsp;&nbsp;&nbsp;</td>';
                                        $html .= '</tr>';
                                    $html .= '</table>';
                                $html .= '</td>';
                            $html .= '</tr>';
                            $html .= '<tr>';
                                $html .= '<td class="nopadding">';
                                    $html .= '<table class="table table-bordered rtoTable">';
                                        $html .= '<tr>';
                                            $html .= '<td class="fixedbg color-two">&nbsp;</td>';
                                            $html .= '<td class="noborder">24-hours</td>';
                                        $html .= '</tr>';
                                    $html .= '</table>';
                                $html .= '</td>';
                            $html .= '</tr>';
                            $html .= '<tr>';
                                ...
                                $html .= '<td class="nopadding">';
                                    $html .= '<table class="table table-bordered rtoTable">';
                                        $html .= '<tr>';
                                            $html .= '<td class="fixedbg color-one">&nbsp;</td>';
                        ...
                        $html .= '</div>';
                    $html .= '<div>'; // WILL BE REMAINED COMMENTED 
                    ...
                    if ( !empty($area['opts']['tap_q1']) ) {
                        $pageBreakAdded = false;
                        for ($i=1; $i <= $area['opts']['tap_q1']; $i++) { 
                            $tap = advisory_form_default_values($postID, $departmentID .'_action_plan_'. $i);
                            if ( !empty($tap) ) {
                                if ( !$pageBreakAdded ) {
                                    $html .= '<div style="page-break-before: always;"></div>';
                                    $pageBreakAdded = true;
                                }
                                // $html .= '<br><pre>'.print_r($tap, true).'</pre>';
                                $html .= '<table class="table table-bordered m0"><tr><td colspan="5" class="people"> TEAM ACTION PLAN ( '.$tap['sop'].' )</td></tr></table>';
                                $html .= '<table class="table table-bordered m0">';
                                    $html .= '<thead>';
                                        $html .= '<tr>';
                                            $html .= '<td class="gray" colspan="2">RESPONSIBLE FOR PLAN INITIATION</td>';
                                            $html .= '<td class="gray">PHONE</td>';
                                            $html .= '<td class="gray">EMAIL</td>';
                                        $html .= '</tr>';
                                    $html .= '</thead>';
                                    $html .= '<tbody>';
                                        $html .= '<tr>';
                                            $html .= '<td class="light-gray" style="width:100px">PRIME</td>';
                                            $html .= '<td class="small">'. $tap['rpi_prime'] .'</td>';
                                            $html .= '<td class="small" style="width:200px">'. $tap['rpi_prime_phone'] .'</td>';
                                            $html .= '<td class="small" style="width:200px">'. $tap['rpi_prime_email'] .'</td>';
                                        $html .= '</tr>';
                                        $html .= '<tr>';
                                            $html .= '<td class="light-gray" style="width:100px">SECONDARY</td>';
                                            $html .= '<td class="small">'. $tap['rpi_secondary'] .'</td>';
                                            $html .= '<td class="small" style="width:200px">'. $tap['rpi_secondary_phone'] .'</td>';
                                            $html .= '<td class="small" style="width:200px">'. $tap['rpi_secondary_email'] .'</td>';
                                        $html .= '</tr>';
                                    $html .= '</tbody>';
                                $html .= '</table>';
                                $html .= '<table class="table table-bordered m0">';
                                    $html .= '<tr>';
                                        $html .= '<td class="gray" style="width:250px;">SERVICE IMPACT/DISRUPTION</td>';
                                        $html .= '<td class="small">'. $tap['sid'] .'</td>';
                                    $html .= '</tr>';
                                $html .= '</table>';
                                $html .= '<table class="table table-bordered">';
                                    $html .= '<tr>';
                                        $html .= '<td class="small" colspan="2" style="width:50%;">'. $tap['ap_day1'] .'</td>';
                                        $html .= '<td class="small" colspan="2" style="width:50%;">'. $tap['ap_day2'] .'</td>';
                                    $html .= '</tr>';
                                $html .= '</table>';
                            }
                        }
        ...
        if (!empty($data)) {
            $html .= '<style>';
                ...
                // BACKGROUND COLORS
                $html .= ' .bg-black {background-color: #000;}';
                ...
                $html .= ' table.headerBottom tr td p strong{font-weight: bold;}';
                $html .= ' table.headerRight {margin: 10px 0 0 0;}';
                $html .= ' table.headerRight tr td{vertical-align:top}';
                $html .= ' table.headerRight tr td p{overflow:hidden;margin: 0 0 5px 0;text-align:center;}';
                $html .= ' table.headerRight tr td p.rounded-bg{height: 50px;width: 50px;line-height: 37px;font-weight:bold;font-size: 14px;margin: 0 auto;}';
                $html .= ' table.headerRight tr td p.rounded-bg{border-radius:25px 25px 25px 25px;margin: 0 auto;}';
                // $html .= ' table.headerRight tr td p.rounded-bg{background-size:100%; background-position:center;background-repeat:no-repeat;}';
                ...
                $html .= ' table.summary tr td.half{width:50%;}';
                $html .= ' table.summary tr td{ padding:0 0 10px 0; vertical-align:top;}';
                $html .= ' table.summary tr td *{font-size:11px;line-height:1;}';
                $html .= ' table.summary tr td p{margin:0 0 5px 0;}';
                $html .= ' table.summary tr th.left, table.summary tr td.left{border-right: 2px solid #000;padding-right:10px;}';
                $html .= ' table.summary tr th.right, table.summary tr td.right{padding-left:10px;}';
                $html .= ' table.tsummary tr td p.title{font-size: 13px; font-weight:bold;margin: 0 0 8px 0;}';
                $html .= ' table.summary tr td ul, table.summary tr td ol{margin-top:0;padding-left:20px;}';
                $html .= ' table.summary tr td ul li, table.summary tr td ol li{margin: 0 0 5px 0;}';
                // ADDITIONAL SKILLS
                ...
                $html .= '<tr>';
                    ...
                    $html .= '<td>';
                        $html .= '<table class="table no-border headerRight">';
                            $html .= '<tr>';
                                $html .= '<td class="text-center skillFit">';
                                    $html .= '<p><strong>SKILLS FIT %</strong></p><p class="rounded-bg '.$data['sfia']['class'].'">'.$data['sfia']['text'].'</p>';
                                $html .= '</td>';
                                $html .= '<td class="text-center technicalScore">';
                                $html .= '<p><strong>TECHNICAL SCORE</strong></p><p class="rounded-bg '.$data['sfiats']['cls'].'">'.$data['sfiats']['txt'].'</p>';
                                // if ( empty($data['sfiats']['summary']) && empty($data['sfiats']['summary_2']) ) $html .= '<p>Not required at this time</p>';
                                $html .= '</td>';
                            $html .= '</tr>';
                        $html .= '</table>';
                    $html .= '</td>';
            ...
            $html .= '</table>';
            // SUMMARY SFIA
            if ( !empty($data['sfia']['summary']) || !empty($data['sfia']['summary_2']) ) {
                $html .= '<table class="table m-0 no-border summary">';
                    $html .= '<tr>';
                        $html .= '<td class="left half">';
                            $html .= $data['sfia']['summary'];
                        $html .= ' </td>';
                        $html .= '<td class="right half">';
                            $html .= $data['sfia']['summary_2'];
                        $html .= ' </td>';
                    $html .= '</tr>';
                $html .= '</table>';
            }
            // SUMMARY TECHNICAL
            if ( !empty($data['sfiats']['summary']) || !empty($data['sfiats']['summary_2']) ) {
                $html .= '<table class="table m-0 no-border summary">';
                    $html .= '<tr>';
                        $html .= '<td class="left half">';
