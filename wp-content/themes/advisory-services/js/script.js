...
jQuery(document).on('click', '.upstreamBtn', function(event) {
	...
	jQuery.ajax({
		...
		beforeSend: function() {},
		success: function(response) {
			jQuery('#modal-bia .modal-title').html(button.attr('data-title'));
...
jQuery(document).on('click', '.ITServiceCatalogue', function(event) {
    var upstream = jQuery(this).attr('services');
	jQuery.ajax({
		url: object.ajaxurl,
		method : 'post',
		data: {
