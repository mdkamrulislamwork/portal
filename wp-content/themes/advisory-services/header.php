        ...
        <nav class="navbar navbar-static-top">
            ...
            <div class="navbar-custom-menu">
                ...
                if (current_user_can('viewer')) {
                    if (is_singular(['csma', 'cma', 'cloud_vendor', 'facility', 'mta', 'ihc', 'itsm', 'cra', 'drm'])) {
        ...
        <section class="sidebar">
            <ul class="sidebar-menu">
                ...
                if (current_user_can('viewer') && advisory_get_user_company_id()) {
                    ...
                    if (!empty($forms)) {
                        if ($user_switching->get_old_user()) {
                            foreach ($forms as $form) {
                                if (get_post_type($form) == 'csa') getActiveCSAMenu($form);
                                ...
                                else if (get_post_type($form) == 'facility') {
                                    $form_meta = get_post_meta($form, 'form_opts', true);
                                    $form_meta['icon'] = IMAGE_DIR_URL.'icon-facility.png';
                                    ...
                                    echo '<li><a href="#"><img src="' . $form_meta['icon'] .'" alt=""><span>'. advisory_get_form_name($form) . '</span></a>';
                                        if (!empty($form_meta['areas'])) {
                                            ...
                                            foreach ( $form_meta['areas'] as $areaSi => $area ) {
                                                $areaIcon = empty($area['icon_menu']) ? $form_meta['icon'] : $area['icon_menu'];
                                                echo '<li><a href="'. $postLink .'?area='. $areaSi .'"><img src="'. $areaIcon .'"><span>'. $area['name'] . '</span></a></li>';
                                            }
                                            echo '</ul>';
                                        }
                                    echo '</li>';
                                    ...
                                    $postLink = get_the_permalink($form);
                                    $icon = '<img src="'.IMAGE_DIR_URL.'icon-cloud_vendor.png">';
                                ...
                                if (get_post_type($form) == 'csa') getActiveCSAMenu($form);
                                    ...
                                    echo '<li><a href="#"><img src="' . $icon .'" alt=""><span>' . advisory_get_form_name($form) . '</span></a>';
                                        if (!empty($form_meta['areas'])) {
                                            echo '<ul class="treeview-menu">';
                                            foreach ($form_meta['areas'] as $areaSi => $area) {
                                                echo '<li><a href="' . get_the_permalink($form) . '?view=true&area='.$areaSi.'"><img src="'.$icon.'"><span>'.$area['name'].'</span></a></li>';
                                            }
                                            echo '</ul>';
                                        }
                                    echo '</li>';
                    ...
                    echo '<li><a href="'. admin_url('edit.php?post_type=page') .'"><img src="' .P3_TEMPLATE_URI.'/images/menu-media.png"> Pages </a></li>';
