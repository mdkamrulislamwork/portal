<?php /* Template Name: Cybersecurity Registry */
get_header();
global $user_switching;
$permission = csmarInputController();
$areaId = !empty($_GET['area']) ? $_GET['area'] : 1;
$threatCatId = 'area_'.$areaId.'_threatcat';
$csmaId = csmaGetActiveOrArchivedAssessmentId();
$opts = get_post_meta($csmaId, 'form_opts', true);
...
$csmrId = advisory_get_user_company_id();
$dateFormat = get_option( 'date_format');
$funcDefault = advisory_form_default_values($csmrId, $threatCatId);
$reportLink = site_url('cybersecurity-report/').'?pid='.base64_encode($csmaId).'&area='.base64_encode($areaId);
?>
<script src="<?php echo P3_TEMPLATE_URI. '/js/plugins/jquery.tinymce.min.js'; ?>"></script>
<div class="content-wrapper">
    <div class="page-title">
        <div> <h1><img class="dashboardIcon" src="<?php echo get_template_directory_uri(); ?>/images/icon-rr.png" alt=""><?php _e('Cybersecurity Maturity Register', 'advisory'); ?></h1> </div>
        <?php 
        if ( $permission['edit'] ) {
            echo '<div>';
                echo '<a class="btn btn-lg btn-success csmr-save-all" data-id="'.$csmrId.'" href="javascript:;">Save All</a>';
            echo '</div>';
        }
        ?>
        <div> <ul class="breadcrumb"> <li><i class="fa fa-home fa-lg"></i></li> <li><a href="#"><?php _e('Cybersecurity Register', 'advisory'); ?></a></li> </ul> </div>
    </div>
    <?php
    if ($csmaId) {
        ...
        $str .= '<div class="row">';
            $str .= '<div class="col-md-12">';
                $str .= '<form class="survey-form" method="post" data-meta="'. $threatCatId .'" data-id="'. $csmrId .'">';
                    $str .= '<div class="card">';
        ...
        if ( !empty($opts[$threatCatId]) ) {
            foreach ( $opts[$threatCatId] as $threatCatSi => $threatCat ) {
                ...
                $csmaDefault = advisory_form_default_values($csmaId, $threatId);
                $default = advisory_form_default_values($csmrId, $threatId);
                $updatedAt = !empty($default['updated_at']) ? date($dateFormat, $default['updated_at']) : 'Please save updates';
                // $str .= '<br>'.$csmrId.' == '.$threatId.'<pre>'.print_r($default, true).'</pre>'; 
                // $str .= '<br>'.$csmaId.' == '.$threatId.'<pre>'.print_r($csmaDefault, true).'</pre>'; 
                $str .= '<div class="row">';
                    $str .= '<div class="col-md-12">';
                        $str .= '<form class="survey-form" method="post" data-meta="'. $threatId .'" data-id="'. $csmrId .'">';
                                ...
                                if ( !empty($opts[$threatId]) ) {
                                    foreach ( $opts[$threatId] as $threatSi => $threat ) {
                                        ...
                                        if ( !empty($opts[$questionId]) ) {
                                            ...
                                            $controlStatementAvg = !empty($csmaDefault[$questionId.'_avg']) ? $csmaDefault[$questionId.'_avg'] : 0;
                                            $controlStatementAvgStatus = csmaAvgStatus($controlStatementAvg);
                                            $controlStatementSummary = csmaCsSummary($opts, $questionId, $csmaDefault);
                                            ...
                                            $str .= '<div class="table-responsive">';
                                                $str .= '<table class="table table-bordered table-survey table-csmaControlStatement">';
                                                        ...
                                                        $str .= '<tr>';
                                                            ...
                                                            $str .= '<td class="'.csmaPriorityBg($defaultPriority).'" style="width: 143px;padding:0;">'. advisory_opt_select($questionId.'_priority', $questionId.'_priority', 'text-center priority', '', $priorities, $defaultPriority) .'</td>';
        ...
        if ( $permission['summary'] ) {
            ...
            $summaryDefault = advisory_form_default_values($csmrId, $summariesId);
            ...
            $str .= '<div class="row">';
                $str .= '<div class="col-md-12">';
                    $str .= '<div class="card">';
                        ...
                        $str .= '<div class="card-body">';
                            $str .= '<form class="survey-form" method="post" data-meta="'.$summariesId.'" data-id="'. $csmrId .'">';
        ...
        echo '<p>' . __('Cybersecurity Maturity Register Not Available Yet') . '</p>';
    } ?>
</div>
<script>
jQuery(function($) {
