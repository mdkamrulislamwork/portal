<?php // Template Name: SFIA Partial Skills Fit
...
if (empty($post_id)) echo '<p>'. __('Partial Skills are Not Available Yet') .'</p>';
else {
    $opts = get_post_meta($post_id, 'form_opts', true);
    $opts['display_name'] = 'SFIA Partial Skills Fit';
...
<div class="content-wrapper">
    <div class="page-title" style="padding-bottom: 5px;">
        ...
        <?php if ($select_attr == '') { 
            echo '<div>';
                echo ' <a class="btn btn-lg btn-success" target="_blank" href="'. site_url('pdf') .'?pid=sfia_partial_skills_fit">Report Preview</a>';
            echo '</div>';
        } ?>
        <div>
            <ul class="breadcrumb">
    ...
    <div class="row">
        <div class="col-sm-12">
            <form class="survey-form" method="post" data-meta="partial_skills_fit" data-id="<?php echo $post_id ?>">
                                ...
                                <div class="col-sm-8 formContainer">
                                    <div class="row">
                                        <div class="col-sm-12 form-group notesContainer">
                                            <?php echo advisory_sfiapsf_get_notes($default, $select_attr); ?>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-sm-3">
                ...
                <?php echo advisory_sfiapsf_get_skills($post_id, $default, $select_attr) ?>
                <?php echo advisory_sfiapsf_get_summary($default, $select_attr) ?>
            </form>
        </div>
    </div>
...
(function($) {
    ...
    function sfiapsf_reset_subcategory(skill) {
        skill.find('.subcategory').html('<option value="">Sub-Category</option>').attr('disabled', true);
    }
    function sfiapsf_reset_skills(skill){
        skill.find('.skills').html('<option value="">Skills</option>').attr('disabled', true);
    }
    function sfiapsf_reset_code(skill){
        skill.find('.code').html('');
        skill.find('.SFIAInfoBtn').attr({'data-skill': '', 'disabled': true});
    }
    function sfiapsf_reset_target_level(skill){
        skill.find('.target_level').html('<option value="">Target</option>').attr('disabled', true);
    }
    function sfiapsf_reset_evaluation_level(skill){
        skill.find('.evaluation_level').html('<option value="">Evaluation</option>').attr('disabled', true);
    }
    function sfiapsf_activate_submit_btn() {
    ...
    function sfiapsf_category_class(skillContainer, value=false) {
        var cls = '';
        if (value) {
            switch (value) {
    ...
    function sfiapsf_recommendation_remove_btn_toggle(summaryWrapper) {
    ...
    updateIndex = function(e, ui) {
        $('td.move', ui.item.parent()).each(function (i) {
            ...
            skill.find('.evaluation_level').attr({'id': 'evaluation_level_'+i, 'name': 'evaluation_level_'+i})
        });
        sfiapsf_activate_submit_btn();
    };
    ...
    $(document).on('change', '.categoryItem', function(e) {
        ...
        if (category.length > 0) {
            sfiapsf_category_class(skill, category);
            ...
            jQuery.ajax({
                ...
                beforeSend: function() { 
                    ...
                    sfiapsf_reset_subcategory(skill);
                    sfiapsf_reset_skills(skill);
                    sfiapsf_reset_code(skill);
                    sfiapsf_reset_target_level(skill);
                    sfiapsf_reset_evaluation_level(skill);
                },
            ...
            jQuery.ajax({
                ...
                success: function(response, status, xhr) {
                    if (response.length > 0) {
                        ...
                        skill.find('.evaluation_level').html('<option value="">Evaluation</option><option value="0">Missing</option>'+response).attr('disabled', false);
                    }
                    button.attr('disabled', false);
                },
                error: function(error) {
    ...
    $(document).on('change', '.evaluation_level, .target_level', function(e) {
        let skillContainer = $(this).parents('.skillContainer');
        let assessed_level = skillContainer.find('.evaluation_level').val();
        let target_level = skillContainer.find('.target_level').val();
        let cls = advisory_get_sfia_evaluation_level_container_class(assessed_level, target_level);
        skillContainer.find('.evaluation_level_container').removeClass().addClass('evaluation_level_container '+ cls);
    })
    $(document).on('click', '.addMoreSkill', function(e) {
        ...
        if (skillCount) {
            jQuery.ajax({
                type: 'POST',
                url: object.ajaxurl + '?action=sfiapsf_add_new_skill',
                cache: false,
    ...
    $(document).on('click', '.SFIARemoveBtn', function(e) {
        ...
        $('.skillWrapper .skillContainer').each(function(index, element) {
            ...
            $(this).find('.evaluation_level').attr({'id': 'evaluation_level_'+index, 'name': 'evaluation_level_'+index});
        })

        sfiapsf_activate_submit_btn();
        sfiapsf_activate_remove_btn();
    })
    // Summary Section
    sfiapsf_summary_remove_btn_toggle($('.summaryWrapper'));
    ...
    sfiapsf_recommendation_remove_btn_toggle($('.recommendationWrapper'));
    ...
    function advisory_get_sfia_evaluation_level_container_class(assessed_level, target_level) {
