...
<?php
$tabletopId = !empty($_GET['id']) ? intval($_GET['id']) : 0;
$data = tabletopGetDataById($tabletopId);
if ( !empty($data) ) :
$permission = tabletopInputController();
$scenarioDesc = !empty($data['scenario']['desc']) ? $data['scenario']['desc'] : '';
?>
<div class="content-wrapper">
    <form id="onlineFormSteps" method="post"<?php echo $permission['attr']; ?>>
                ...
                <div class="form-items">
                    <table class="table table-bordered mb-0">
                        <tr>
                            ...
                            <td class="bg-gray" style="width: 160px;">Test Method:</td>
                        ...
                        <tr>
                            ...
                            <?php if ( empty($permission['attr']) ): ?>
                                <td style="width: 50px;" class="no-padding signatureWrapper">
                                    <button id="upload_signature" class="btn btn-primary btn-md btn-block" type="button"><span class="fa fa-upload"></span></button>
                                    <button class="btn btn-danger btn-md btn-block removeBtn" type="button"<?php echo empty($data['signature']) ? ' disabled' : '' ?>><span class="fa fa-trash"></span></button>
                            ...
                            <td style="width: 300px;" class="no-padding text-center">
                                <img class="imagePreview" style="max-height:50px;" src="<?php echo !empty($data['signature']) ? $data['signature'] : 'http://via.placeholder.com/200x50?text=Signature' ?>">
                            </td>
                        </tr>
                    </table>
                </div>
...
jQuery(function($) {
    ...
    const tabletopId = <?php echo $tabletopId; ?>;
    ...
    $("#onlineFormSteps").steps({
        ...
        onStepChanged: function (event, currentIndex, priorIndex) {
            if ( isUpdateable ) updateTableTopForm();
        },
        onFinishing: function (event, currentIndex) {
            if ( isUpdateable ) updateTableTopForm();
            tabletopModal.modal('hide');
            if ( isUpdateable ) swal('success', 'Tabletop data is saved successfully.');
            setTimeout(function() {
    ...
    // SUPPORT FUNCTIONS
    function updateTableTopForm(isFinalStep=false) {
        var form = $('#onlineFormSteps');
        var formData = form.serialize();
        jQuery.ajax({
            type: 'POST',
            // url: object.ajaxurl + '?action=table_top',
            url: object.ajaxurl + '?action=tabletop_update',
            cache: false,
            data: {id: tabletopId, data: formData, security: object.ajax_nonce },
    ...
    $(document).on( 'click', '.removeBtn', function () {
        $('#signature').val('');
        $('.imagePreview').prop('src', 'http://via.placeholder.com/200x50?text=Signature');
