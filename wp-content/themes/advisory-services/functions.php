...
require_once P3_TEMPLATE_PATH .'/helper/facility.php';
require_once P3_TEMPLATE_PATH .'/helper/cloud_vendor.php';
require_once P3_TEMPLATE_PATH .'/helper/cma.php';
require_once P3_TEMPLATE_PATH .'/helper/csma.php';
...
function advisory_ajax_reset_survey() {
	...
	// wp_send_json( advisory_has_survey_delete_permission($form_id) );
	...
	} else if($postType == 'bcp' || $postType == 'facility' || $postType == 'cloud_vendor'){
		$sections = get_post_meta($form_id);
		if (!empty($sections)) {
			foreach ($sections as $key => $section) {
				parse_str($section[0], $section);
	...
	} else if( $postType == 'cma' ) {
		$opts = get_post_meta($form_id, 'form_opts', true);
		if ( !empty($opts['areas']) ) {
			foreach ($opts['areas'] as $areaId => $threatsCats) {
				...
				if ( !empty($opts[$threatCatId]) ) {
					foreach ( $opts[$threatCatId] as $threatCatSi => $threatCat ) {
						...
						$deleted[] = delete_post_meta($form_id, $threatId);
					}
				}
			}
			if (!empty($deleted)) wp_send_json( true );
...
function advisory_ajax_dashboard_scorecard() {
	...
	if ($requestedPostType == 'bia') {
        ...
        $html .= '<div class="text-center"><img src="'.P3_TEMPLATE_URI.'/images/value-scorecard-risk.jpg" class="img-responsive"></div><br>';
        ...
        if ($requestedPostType == 'bcp' || $requestedPostType == 'facility') {
        	$scoreType = $requestedPostType == 'bcp' ? 'bcp-score' : 'risk-score';
        	$heatLoop = $requestedPostType == 'bcp' ? 126 : 17;
	    ...
	    $html .= '<div class="card-header-tabs-line scorecard-tab pb-8">';
	        $html .= '<div class="custom-title">';
				...
				$html .= '<h6 class="sub-title"><span>Facility Risk Assessment</span></h6>';
			$html .= '</div>';
		$html .= '</div>';
        // $html .= '<div class="text-center"> <img src="'.P3_TEMPLATE_URI.'/images/facility/scorecard_top.png" class="img-responsive"> </div><br>';
    	$heatSteps = facilityHeatMap($char_arr);
    	// $html .= '<br><pre>'. print_r($heatSteps, true) .'</pre>';
	    if (!empty($heatSteps)) {
	        $html .= '<div class="row">';
		        $html .= '<div class="col-sm-12">';
		            $html .= '<div class="score-table mb clearfix">';
		                $html .= '<ul class="heatIndex">';
		                    $html .= '<li class="t-heading-dark" style="width:16.66%"> <strong>Risk Heat Map</strong> </li>';
	                		foreach ($heatSteps as $heatStep) {
	                			$html .= '<li class="'. $heatStep['color'] .'" style="width:16.66%"> <strong>' . $heatStep['range'] . '</strong> ' . $heatStep['char'] . '</li>';
	                		}
		                    $html .= '<li class="t-heading-dark" style="width:16.66%"> <strong>Risk Heat Map</strong> </li>';
		                $html .= '</ul>';
		            $html .= '</div>';
	                    ...
	                    if ($tmp_threats) {
	                        ...
	                        foreach ($tmp_threats as $threat) {
	                            $html .= '<tr>
	                                <td>' . $char_arr[$threat['avg']] . '</td>
	                                <td>' . $threat['name'] . ' (' . $threat['cat'] . ')</td>
	                                <td><span class="box-padding '. facilityRiskBackground($threat['avg']).'"></span>&nbsp;&nbsp;' . $threat['avg'] . '</td>
	                            </tr>';
	                        }
	                    }
	                $html .= '</tbody>
		...
		if ( !empty($data) ) {
			$html .= '<div class="card-header-tabs-line scorecard-tab pb-8">';
		        $html .= '<div class="custom-title">';
					$html .= '<h1 class="title">Value scorecard</h1>';
					$html .= '<h6 class="sub-title"><span>Cloud Maturity Assessment</span></h6>';
				$html .= '</div>';
			$html .= '</div>';
			$html .= '<div class="table-responsive">';
			    $html .= '<table class="table table-bordered table-survey table_others">';
			        $html .= '<tbody>';
						foreach ($data as $areaId => $area) {
							$areaDesc = !empty($area['desc']) ? '<br><small>'.$area['desc'].'</small>' : '';
				            $html .= '<tr>';
				                $html .= '<th colspan="2" class="bg-'.$area['color'].' color-white"><big>Category : '.$area['name'].'</big> '.$areaDesc.'</th>';
				            $html .= '</tr>';
				            if ( !empty($area['domains'])) {
					            $html .= '<tr>';
					                $html .= '<th class="t-heading-dark"><big>Domain</big></th>';
					                $html .= '<th class="t-heading-dark text-center" style="width:80px;">Maturity Rating</th>';
					            $html .= '</tr>';
				            	foreach ($area['domains'] as $domain) {
				            		$avg = !empty($domain['avg']) ? $domain['avg'] : 0;
						            $html .= '<tr>';
						                $html .= '<th class="">'.$domain['name'].' <small>'.$domain['desc'].'</small></th>';
...
function advisory_scorecard_header($subTitle='')
{
	$str = '';
	$str .= '<div class="card-header-tabs-line scorecard-tab pb-8">';
        $str .= '<div class="custom-title">';
			$str .= '<h1 class="title">Value scorecard</h1>';
			$str .= '<h6 class="sub-title"><span>'.$subTitle.'</span></h6>';
		$str .= '</div>';
	$str .= '</div>';
	return $str;
}
...
function advisory_registered_criteria($name = 'ihc'): array{
	...
	elseif($name == 'facility') $criteria = cs_get_option('criteria_risk');
...
function advisory_get_formatted_rr_data($post_id, $postType=null) {
	if ( !$postType ) $postType = get_post_type($post_id);
	$meta = get_post_meta($post_id, 'form_opts', true);
	$data = [];
	$index = 0;
	if ( $postType == 'facility' && !empty($meta['areas']) ) {
...
function advisory_get_scorecard_data($id) {
	...
	$count = 0;
	if ( !empty($form_data['areas']) ) {
		foreach ($form_data['areas'] as $areaSi => $area) {
			$count2 = 0;
			if ($requestedPostType == 'bia') {
				$data[$count]['name'] = $area['name'];
				...
				if (!empty($form_data[$services])) {
                    foreach ($form_data[$services] as $service) {
                    	if ( !empty($transient_data[$services . '_bia_' . advisory_id_from_string($service['name'])]) ) {
							$data[$count]['services'][$count2]['name'] = $service['name'];
							$service_data =  $transient_data[$services . '_bia_' . advisory_id_from_string($service['name'])];
							parse_str($service_data[0], $service_data);
							$data[$count]['total'] += !empty($service_data['avg']) ? $service_data['avg'] : 0;
							$data[$count]['services'][$count2]['cr'] = isset($service_data['avg']) ? $service_data['avg'] : null;
							$data[$count]['services'][$count2]['rto'] = isset($service_data['rto']) ? $service_data['rto'] : null;
				...
				if ( $threatsCats = @$form_data[$threatCatId] ) {
					foreach ( $threatsCats as $threatCatSi => $threatCat ) {
						$threatId = $threatCatId.'_'.$threatCatSi.'_threat';
						$threat_data = !empty($transient_data[$threatId]) ? $transient_data[$threatId] : null;
						if ( $threats = @$form_data[$threatId] ) {
							parse_str($threat_data[0], $threat_data);
							$data[$count]['threats'][$count2]['name'] = $threatCat['name'];
							$data[$count]['threats'][$count2]['cat']  = $area['name'];
							$data[$count]['threats'][$count2]['avg']  = round($threat_data['avg']);
						}
						$count2++;
				...
				$data[$count]['color'] = cmaCategoryColor($areaSi);
				$threatCatId = 'area_'.$areaSi . '_threatcat';
				if ( $threatsCats = $form_data[$threatCatId] ) {
					foreach ( $threatsCats as $threatCatSi => $threatCat ) {
						if ( !empty($threatCat['name']) ) {
							...
							$data[$count]['domains'][$count2]['name'] = $threatCat['name'];
							$data[$count]['domains'][$count2]['desc'] = !empty($threatCat['desc']) ? '( '.$threatCat['desc'].' )' : null;
							$data[$count]['domains'][$count2]['avg'] = !empty($default['avg']) ? $default['avg'] : 0;
							// $data[$count]['domains'][$count2]['test'] = $default;
							$count2++;
						}
					}
				}
				...
				if ( $threatsCats = $form_data[$threatCatId] ) {
					foreach ( $threatsCats as $threatCatSi => $threatCat ) {
						if ( !empty($threatCat['name']) ) {
							$data[$areaSi]['threatcats'][$threatCatSi]['name'] = $threatCat['name'];
							$threatId = $threatCatId.'_'.$threatCatSi.'_threat';
							if ( !empty($form_data[$threatId]) ) {
				                foreach ($form_data[$threatId] as $threatSi => $threat) {
					                $questionId = $threatId .'_'.$threatSi.'_question';
				                	if ( !empty($form_data[$questionId]) ) {
			            				$default = advisory_form_default_values($id, $questionId);
					                	$data[$areaSi]['threatcats'][$threatCatSi]['threats'][$threatSi]['name'] = $threat['name'];
					                	$data[$areaSi]['threatcats'][$threatCatSi]['threats'][$threatSi]['avg'] = !empty($default['avg']) ? $default['avg'] : null;
					                	$data[$areaSi]['threatcats'][$threatCatSi]['threats'][$threatSi]['color'] = @cloudVendorAvgColor($default['avg']);
					                	// $data[$areaSi]['threatcats'][$threatCatSi]['threats'][$threatSi]['test'] = $default;
				                	}
				                }
				            }
						}
...
function getArchivedMenuFor($postType='risk', $title='DR Risk Register', $pageTitle='Risk Register', $icon=null, $showEmpty=false) {
    ...
    if (!empty($rr_form_id[0])) {
        ...
        // if ( $postType == 'facility' ) { echo '<br><pre>'.print_r($rr_data, true).'</pre>'; }
...
function inputPermission($userType='')
{
    global $user_switching;
    $data = [
        'publish_btn'   => false,
    ...
    $specialUser    = $userType ? get_the_author_meta( $userType, $userId ) : null;
...
add_filter( 'ajax_query_attachments_args', 'advisory_show_current_user_attachments' );
function advisory_show_current_user_attachments( $query ) {
	global $user_switching;
