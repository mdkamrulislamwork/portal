    ...
    <?php wp_footer(); ?>
	<?php if (is_page('dashboard') && is_user_logged_in() && !is_admin() ) { 
		$data = get_term_meta(advisory_get_user_company_id(), 'company_data', true);
		// echo '<br><pre style="margin-left:300px;">'.print_r($data['mta_static'], true).'</pre>';
		$dashboard = @$data['userDashboard'] == 'Dashboard B' ? true : false;
		if ($dashboard) : 
			// $chartData = advisory_mta_chart_data();
			if ( !empty($chartData = $data['mta_static']) ) :
			?>
			...
			<?php endif; ?>
	<?php else : 
		if ( !empty($chartData = $data['static']) ) : ?>
		    <script type="text/javascript">
				var config = {
		            type: 'line',
		            data: {
	...
	<?php endif; ?>
	<?php } ?>
	<?php if (is_page('itscm') && is_user_logged_in()) { 
		$tid = advisory_get_user_company_id(); 
		$data = get_term_meta(advisory_get_user_company_id(), 'company_data', true);
	    ...
	    <script type="text/javascript">
			var config = {
	            ...
	            data: {
	                ...
	                datasets: [
	                <?php 
	                if ( !empty($data['dr_static']) ) {
		                foreach ($data['dr_static'] as $static) {
		                	echo '{
