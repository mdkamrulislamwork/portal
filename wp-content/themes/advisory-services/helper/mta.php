...
function advisory_generate_mta_form_sections() {
			...
			foreach ($meta['areas'] as $area) {
				$data[] = array(
					...
					'fields' => [
						...
						['id' => 'docs', 'type' => 'upload', 'title' => 'Documentation']
					],
				);
			}
		}
...
function advisory_generate_mta_form_tables() {
	if (is_admin() && is_edit_page()) {
		$id = @$_GET['post'];
		$meta = get_post_meta($id, 'form_opts', true);
		$data = [];
...
function advisory_generate_mta_form_questions() {
	if (is_admin() && is_edit_page()) {
		$id = @$_GET['post'];
		$meta = get_post_meta($id, 'form_opts', true);
		$tables = [];
...
function advisory_generate_mta_pdf_subsections($postType='mta') {
	if (is_admin()) {
		$data = [];
		$id = 0;
		$prefix = 'mta_pdf_';
		...
		if ($query->found_posts > 0) {
			$id = $query->posts[0]; 
			$meta = get_post_meta($id, 'form_opts', true);
			$data[] = ['type' => 'heading', 'content' => 'MTA PDF DEFAULT INPUT']; 
					...
					if (!empty($meta[$sectionID])) {
						foreach ($meta[$sectionID] as $section) {
							$tableID = $sectionID. '_tables_' . advisory_id_from_string($section['name']);
								...
								$data[] = ['id' => $assessmentTitleID, 'type' => 'textarea', 'title' => 'Assessment Title', 'default' => demoContent('title',2)];
								$data[] = ['id' => $assessmentDescID, 'type' => 'textarea', 'title' => 'Assessment Desc', 'default' => demoContent('list', 6)];
								$data[] = ['id' => $summaryTitleID, 'type' => 'textarea', 'title' => 'Summary Title', 'default' => demoContent('title')];
...
function getMtaAreas($postID, $title='') {
	$data = [];
	// echo '<option value="'. get_the_ID() .'">'. $title .'</option>';
	$meta = get_post_meta($postID, 'form_opts', true);
	if (!empty($meta['areas'])) {
		foreach ($meta['areas'] as $area) {
			$data[advisory_id_from_string($area['name'])] = $area['name'];
		}
	}
	return $data;
}