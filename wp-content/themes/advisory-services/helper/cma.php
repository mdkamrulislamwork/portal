					...
					foreach ( $threatsCats as $threatCatSi => $threatCat ) {
						if ( !empty($threatCat['name']) ) {
							$data[] = array(
								...
								'accordion_title' => 'Add New',
								'fields' => [
                                    ['id' => 'name', 'type' => 'textarea', 'title' => 'Name'],
                                    ['id' => 'cmm_0', 'type' => 'textarea', 'title' => 'CMM 0', 'default' => 'CMM 0'],
                                    ['id' => 'cmm_1', 'type' => 'textarea', 'title' => 'CMM 1', 'default' => 'CMM 1'],
                                    ['id' => 'cmm_2', 'type' => 'textarea', 'title' => 'CMM 2', 'default' => 'CMM 2'],
                                    ['id' => 'cmm_3', 'type' => 'textarea', 'title' => 'CMM 3', 'default' => 'CMM 3'],
                                    ['id' => 'cmm_4', 'type' => 'textarea', 'title' => 'CMM 4', 'default' => 'CMM 4'],
                                    ['id' => 'cmm_5', 'type' => 'textarea', 'title' => 'CMM 5', 'default' => 'CMM 5'],
                                    ['id' => 'info', 'type' => 'textarea', 'title' => 'Info'],
                                ]
							);
						}
					}
...
function cmaBackgroundByValue($val, $type='impact', $reverse=false) 
{
	$cl = 'bg-blue';
...
function cma_risk_calc($vulnerability, $impact, $probability) {
...
function cmaRiskBackground(float $value) {
...
function cmaHeatMap($char_arr) 
{
    ...
    foreach ($steps as $stepSI => $step) {
        ...
        $data[$stepSI]['color'] = cmaRiskBackground($step); 
...
function cma_get_formatted_data($post_id) {
                ...
                foreach ( $threatsCats as $threatCatSi => $threatCat ) {
                    ...
                    if ($default) {
                        ...
                        foreach ($default as $key => $value) {
                            if ( cmaRegisterDefaultItemName($key) == 'impact' )        { $data[$index]['areas'][$index2]['impact'][] = $value; }
                            if ( cmaRegisterDefaultItemName($key) == 'vulnerability' ) { $data[$index]['areas'][$index2]['vulnerability'][] = $value; }
                            if ( cmaRegisterDefaultItemName($key) == 'threat' )        { $data[$index]['areas'][$index2]['threat'][] = $value; }
                        }
                    }
                }
            }
...
function cmaRegisterDefaultItemName($key)
{
    if ( $key ) {
        $key = explode('_', $key);
...
function cmaDomainMap($opts)
{
    ...
    if ( !empty($opts['areas']) ) {
        ...
        $data = [];
        $postLink = site_url('cma/'. $post->ID).'/?area=';
        ...
        $str .='<img src="'. IMAGE_DIR_URL .'cma/banner-left2.jpg" class="img-responsive text-center" usemap="#image-map" hidefocus="true" />';
        ...
        foreach ( $opts['areas'] as $areaSi => $value) {
            if ( empty($opts['areas'][$areaSi]['name']) ) $data[$areaSi] = $defaultItem;
                ...
                $data[$areaSi] = [
                    ...
                    'color' => cmaCategoryColor($areaSi),
                    'image' => IMAGE_DIR_URL.'cma/icon-'. $areaSi .'.png',
...
function cmaDomainCMMBackground($itemSi=0, $value=null)
{
...
function cmaCategoryColor($categorySi=0)
{
...
function cmaInputController()
{
    ...
    $specialUser    = get_the_author_meta( 'specialcmauser', $userId);
...
function cmaCmmModal()
{
    $str = null;
    $str .= '<div id="modal-cma" class="modal fade">';
...
function cmaSummaryModal()
{
    $str = null;
    $str .= '<div id="cmaSummaryModal" class="modal fade">';
