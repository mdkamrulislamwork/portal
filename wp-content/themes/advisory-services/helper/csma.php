...
function csmaBackgroundByValue($val, $type='impact', $reverse=false) 
{
    $cl = 'bg-blue';
...
function csma_risk_calc($vulnerability, $impact, $probability) {
...
function csmaRiskBackground(float $value) {
...
function csmaHeatMap($char_arr) 
{
    ...
    foreach ($steps as $stepSI => $step) {
        ...
        $data[$stepSI]['color'] = csmaRiskBackground($step); 
...
function csma_get_formatted_data($post_id) {
                ...
                foreach ( $threatsCats as $threatCatSi => $threatCat ) {
                    ...
                    if ($default) {
                        ...
                        foreach ($default as $key => $value) {
                            if ( csmaRegisterDefaultItemName($key) == 'impact' )        { $data[$index]['areas'][$index2]['impact'][] = $value; }
                            if ( csmaRegisterDefaultItemName($key) == 'vulnerability' ) { $data[$index]['areas'][$index2]['vulnerability'][] = $value; }
                            if ( csmaRegisterDefaultItemName($key) == 'threat' )        { $data[$index]['areas'][$index2]['threat'][] = $value; }
                        }
                    }
                }
            }
...
function csmaRegisterDefaultItemName($key)
{
    if ( $key ) {
        $key = explode('_', $key);
...
function csmaDomainMap($opts)
{
    ...
    if ( !empty($opts['areas']) ) {
        ...
        $data = [];
        $postLink = site_url('csma/'. $post->ID).'/?area=';
...
function csmaDomainCMMBackground($itemSi=0, $value=null)
{
...
function csmaCategoryColor($categorySi=0)
{
...
function csmaAvgBackground($avg)
{
    $text = $cls = '';
...
function csmaSidebarMenu($form, $areaParam=null)
{
    if ( $form ) {
        global $user_switching;
        $str = null;
        $form_meta = get_post_meta($form, 'form_opts', true);
        $icon = '<img src="'.IMAGE_DIR_URL.'csma/icon-menu.png" alt="">';
        ...
        // $specialUser    = get_the_author_meta( 'specialcsmauser', $userId );
        // $specialUser    = 0;
...
function csmaFunctionBackground($functionId = 1)
{
    $data = null;
...
function csmaResponseBg(int $response=0)
{
    if ( $response == 1 ) return 'bg-green';
    else if ( $response == 3 ) return 'bg-yellow';
    else return 'bg-red';
}
function csmaCsSummary($opts, $questionId, $default)
{
    $str = null;
...
add_action('wp_ajax_reset_csma', 'csma_ajax_reset');
function csma_ajax_reset() 
{
...
function csmaRegisterData($post_id)
{
    $data = [];
    $meta = get_post_meta($post_id, 'form_opts', true);
    if (!empty($meta['areas'])) {
...
function csmaGetActiveOrArchivedAssessmentId()
{
    $company_id = advisory_get_user_company_id();
    if (advisory_metrics_in_progress($company_id, array('csma'))) {
        $form_id = advisory_get_active_forms($company_id, array('csma'));
    } else {
        $id = new WP_Query([
            'post_type'         => 'csma',
            'post_status'       => 'archived',
...
function csma_pdf_get_data($postId=0, $areaId=0)
{
    ...
    if ( $postId && $areaId ) {
        ...
        $headerBg = csmaFunctionBackground($areaId);
