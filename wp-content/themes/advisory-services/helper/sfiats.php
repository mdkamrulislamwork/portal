...
function advisory_sfiats_get_skills_history($user_id, $company_id, $default=[]) {
    ...
    $class = !empty($posts->posts) ? 'bg-red' : 'bg-gap';
    $data .= '<td class="'.$class.' p-0" style="width:300px;">';
...
function advisory_get_sfiats_avg_class($avg) {
    $cls = '';
    // if ( !is_numeric($avg) ) { $cls = 'bg-black color-white'; }
    if ($avg >= 100) 	{ $cls = 'bg-deepblue'; }
...
function advisory_get_sfiats_avg_text($avg) {
	return number_format($avg).'%';
    if ( !is_numeric($avg) ) { return 'N/A'; }
    else return number_format($avg).'%';
}
function advisory_get_sfiats_avg_class_and_text($sfiats_id) {
	$data = null;
	if ( empty($sfiats_id) ) {
		$data['txt'] = 'N/A';
		$data['cls'] = 'bg-black color-white';
	} else {
		$sfiats = advisory_form_default_values($sfiats_id, 'poles');
		$data['txt'] = advisory_get_sfiats_avg_text(@$sfiats['avg']);
		$data['cls'] = advisory_get_sfiats_avg_class(@$sfiats['avg']);
	}
	return $data;
}