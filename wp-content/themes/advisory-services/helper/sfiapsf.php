<?php
function advisory_sfiapsf_get_notes($default,  $select_attr='disabled') {
	$data = '';
...
function advisory_sfiapsf_get_summary($default, $select_attr='disabled') {
	$data = '';
...
function advisory_sfiapsf_get_skills($post_id, $default, $select_attr='disabled') {
	$data = '';
	if ( !empty($default) ) {
		unset($default['time']);
		unset($default['notes']);
		...
		$data .= '<table class="table-bordered skillWrapper">';
	        $data .= '<thead>';
	            $data .= '<tr class="titleWrapper">';
	                ...
	                $data .= '<td class="targetLevelTitle"> <h3 class="title">Evaluation</h3> </td>';
	                $data .= '<td class="actionTitle"><h3 class="title">Actions</h3></td>';
	            $data .= '</tr>';
	        $data .= '</thead>';
	        $data .= '<tbody id="sortable">';
	        $data .= advisory_sfiapsf_get_skill_items($opts, 0, $default, $select_attr);
	        $data .= '</tbody>';
	    $data .= '</table>';
...
function advisory_sfiapsf_get_skill_items($opts, $skillCount=0, $default=[], $select_attr='', $ajax_call=false) {
	$html = '';
	$loop = !empty($default) ? count($default) / 5 : 0;
	...
	do {
		...
		$defaultCatClass= advisory_get_sfiapsf_category_class(@$default['category_'.$skillCount]);
		$html .= '<tr id="skillContainer_'.$skillCount.'" class="skillContainer '.$defaultCatClass.'" skillcount='.$skillCount.'>';
	        ...
	        $html .= '<td class="target_level_container">';
	        	// $html .= '<pre>'.print_r($level_opts, true).'</pre>';
	        ...
	        $default_evaluation_level = isset($default['evaluation_level_'.$skillCount]) ? $default['evaluation_level_'.$skillCount] : '';
	        $default_assess_level_class = advisory_get_sfia_access_level_container_class($default_evaluation_level, $default_target_level);
	        $html .= '<td class="evaluation_level_container '.$default_assess_level_class.'">';
	        	// $html .= '<pre>'.print_r($level_opts, true).'</pre>';
	            $html .= '<select name="evaluation_level_'.$skillCount.'" id="evaluation_level_'.$skillCount.'" class="evaluation_level" style="text-align-last: center;" '.$skillAttr.'>';
	                $html .= '<option value="">Evaluation</option>';
	                if (!empty($level_opts)) {
	                	$level_opts = [0=>'Missing'] + $level_opts;
	                	foreach ($level_opts as $levelId => $level) {
	                		$selectedLevel = $default_evaluation_level == $levelId ? 'selected' : '';
	                		$html .= '<option value="'.$levelId.'" '.$selectedLevel.'>'.$level.'</option>';
	                	}
	                }
	            $html .= '</select>';
...
add_action('wp_ajax_sfiapsf_add_new_skill', 'sfiapsf_add_new_skill');
function sfiapsf_add_new_skill() {
	$html = '';
	$post_id = !empty($_POST['post_id']) ? $_POST['post_id'] : false;
	$counter = !empty($_POST['counter']) ? $_POST['counter'] : false;
	if ($post_id && $counter) {
		$opts = get_post_meta($post_id, 'form_opts', true);
		$html .= advisory_sfiapsf_get_skill_items($opts, $counter, [], '', true);
	}
	echo $html; wp_die();
}
function advisory_get_sfiapsf_category_class($value) {
	$cls = '';
	switch ($value) {
...
function advisory_sfiapsf_dashboard_scorecard() {
	$html = '';
	$skills = advisory_sfiapsf_dashboard_scorecard_skills();
	if ( !empty($skills) ) {
...
function advisory_sfiapsf_dashboard_scorecard_skills($user_company_id=null) {
	$skills = [];
	...
	for ($counter=0; $counter < $loop; $counter++) { 
		...
		$skills[ $default['category_'.$counter] ][ $default['subcategory_'.$counter] ][$counter] = [
			...
			'evaluation' => $default['evaluation_level_'.$counter],
		];
	}
	return $skills;
}
function advisory_sfiapsf_pdf_skills($opts, $default=null) {
	$skills = [];
	$loop = !empty($default) ? count($default) / 5 : 0;
