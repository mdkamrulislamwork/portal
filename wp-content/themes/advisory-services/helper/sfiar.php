...
function advisory_sfiar_get_menu_items($post_id, $company_id=0) {
	...
	if ($sfia_premission) {
		...
		$icon = '<img src="'.P3_TEMPLATE_URI.'/images/registers/icon-purple-dark.png" alt="'. @$title .' Icon">';
	    ...
	    if (!empty($teams)) {
	        $data .= '<li><a href="javascript:;">'.$icon.'<span>' . advisory_get_form_name($post_id) . '</span></a>';
	            if (!empty($teams)) {
	                $data .= '<ul class="treeview-menu">';
	                foreach ($teams as $team_id => $team_name) {
	                    $data .= '<li><a href="'.home_url('sfia-register/').'?team='.$team_id.'">'.$icon.'<span>'.$team_name.'</span></a></li>';
	                }
	                $data .= '</ul>';
	            }
	        $data .= '</li>';
                ...
                $data .= '<table class="table table-bordered m-0" style="width:1000px;">';
                    ...
                    $data .= '<tbody>';
                        ...
                        $data .= '<tr>';
                            ...
                            $data .= '<td class="text-center bg-red p-0" style="width:300px;"><a href="'. home_url('/pdf/?pid='.$sfia_id.'&user_id='.$user_id) .'" target="_blank" style="color:black;display: block;padding: 10px;">Active SFIA Report</a></td>';
                            $data .= $published_sfiats_select_option;
                        $data .= '</tr>';
                    $data .= '</tbody>';
                $data .= '</table>';
                $data .= '<br>';
	...
	$data .= '<table class="table table-bordered m-0">';
        $data .= '<tbody>';
            ...
            for ($i=0; $i < $loop; $i++) { 
				...
				$accessLevel = $skills['assess_level_'.$i] ? 'Level '.$skills['assess_level_'.$i] : 'Missing';
                ...
                $data .= '<tr>';
                    ...
                    $data .= '<td class="'.$class.'" style="width: 110px;">'.$accessLevel.'</td>';
                $data .= '</tr>';
			}
        $data .= '</tbody>';
    $data .= '</table>';
