...
function advisory_sfia_get_team_user_details_notes($head, $unique_id, $select_attr) {
    ...
    $data .= '<textarea name="notes" cols="30" rows="4" class="form-control" placeholder="Notes"'.$select_attr.' style="resize:none">'.$head['notes'].'</textarea>';
...
function advisory_sfia_get_team_technical_score_avg($unique_id, $head, $select_attr, $archive=false) {
	...
	if ($archive) {
		$sfiats_id = isset($head['sfiats_id']) ? $head['sfiats_id'] : false ;
	...
	// $data .='<br><pre>'. print_r($sfiats_id, true) .'</pre>';
	if ( empty($sfiats_id) ) {
		$sfiats['cls'] = 'bg-black color-white';
		$sfiats['text'] = 'N/A';
		$sfiats['img_url'] = IMAGE_DIR_URL.'sfia/assessment/start-red.png';
		$sfiats['link'] = 'href="javascript:;"';
	} else {
		$default = advisory_form_default_values($sfiats_id, 'poles');
		$sfiats['cls'] = advisory_get_sfiats_avg_class($default['avg']);
		$sfiats['text'] = advisory_get_sfiats_avg_text($default['avg']);
		if ( get_post_status($sfiats_id) == 'archived' ) {
			$sfiats['img_url'] = IMAGE_DIR_URL.'sfia/assessment/start-red.png';
			$sfiats['link'] = 'href="javascript:;"';
		} else {
			$sfiats['img_url'] = IMAGE_DIR_URL.'sfia/assessment/start-green.png';
			$sfiats['link'] = 'href="'.site_url('sfiats/').$sfiats_id.'/" target="_blank"';
		}
	}
...
function advisory_sfia_get_team_user_summary_sampledata($part=1) {
	$summary = '<p><strong>NOTE:</strong> The analyst confirmed that the "Programmer" component of the job title is inaccurate - there is little, if any, programming not at the Board. It would be better descibed as a Database Administrator for applications primarily used in Human Resources (iSYS, SmartFindExpress). Provides excessive amount of support to the DBA - DBA not able to fulfill that role as required within the current job description. The analyst might have authorization to the database that is not recommended for someone in this role.</p> <p><strong>NOTE:</strong> CIO tends to micro-manage this group which seems to be creating a bottleneck in getting work done within this particular group (applications)</p> <p><strong>NOTE</strong>: Requests are often going directly to Level-2 application support and not flowing through the proper channels. There could be a lack of support within the HR department causing a trickle effect to the application group who now need to help with budget codes, location codes etc.&nbsp;</p> <ul> <li><strong>Business Analysis</strong> - Investigates operational requirements, problems, and opportunities, seeking effective business solutionsImprovements in automated and non-automated components of new or changed processes. <ul> <li>Assists in the analysis of stakeholder objectives, and the underlying issues arising from investigations into business requirements and problems, and identifies options for consideration.<br>Works with stakeholders, to identify potential benefits and available options for consideration, and in defining acceptance tests.</li> <li>EXAMPLE: works with HR (business systems) to help with work flows, new applications (helping with hiring processes and records management)</li> </ul> </li> <li><strong>Data Modelling and Design -</strong> Applies data analysis, design, modelling, and quality assurance techniques, based upon a detailed understanding of business processes. <ul> <li>Establishes, modifies, maintains data structures and associated components (entity descriptions, relationship descriptions, attribute definitions).<br>Advises application team members on the details of data structures and associated components.</li> <li>EXAMPLE: pulling reports, providing analysis, data flow (how information is being inputted into the HR system - iSYS)</li> <li>EXAMPLE: Data flows - SSIS (Microsoft) - data integration and workflow applications (new applications that require data to integrate into another application).</li> </ul> </li></ul>'; 
	$summary_2 = '<ul> <li><strong>Testing -</strong> Responsible for the creation of test cases using technical analysis of both functional and non-functional specifications (such as reliability, efficiency, usability, maintainability and portability). <ul> <li>Creates traceability records, from test cases back to requirements.</li> <li>Produces test scripts, materials and regression test packs to test new and amended software or services.</li> <li>Specifies requirements for environment, data, resources and tools.</li> <li>Interprets, executes and documents complex test scripts using agreed methods and standards.</li> <li>Records and analyses actions and results, and maintains a defect register.</li> <li>Reviews test results and modifies tests if necessary. Provides reports on progress, anomalies, risks and issues associated with the overall project.</li> <li>Reports on system quality and collects metrics on test cases. Provides specialist advice to support others.</li> </ul> </li> <li><strong>Application Support -</strong> Maintains application support processes, and checks that all requests for support are dealt with according to agreed procedures.Uses application management software and tools <ul> <li>investigate issues, collect performance statistics and create reports.</li> </ul> </li> <li><strong>Database Administration</strong> - The analyst is supporting the database at a level far above the actual requirements by completing tasks that should be exclusive to the DBA (LEVEL-5)Identifies, evaluates and manages the adoption of appropriate database administration tools and processes, including automation. <ul> <li>Contributes to the setting of standards for definition, security and integrity of database objects and ensures conformance to these standards.</li> <li>Manages database configuration including installing and upgrading software and maintaining relevant documentation.</li> <li><strong>EXAMPLE:</strong> The analyst provides support including security updates, table support, view creation, adding roles, etc. (DBA responsibilities) &nbsp;</li> </ul> </li> <li><strong>Supplier Management</strong> - Acts as the routine contact point between the organization and application vendor (iSYS). <ul> <li>Supports resolution of supplier related incidents, problems, or unsatisfactory performance.Collects and reports on supplier performance data (iSYS).</li> </ul> </li> <li><strong>Customer Service Support</strong> - Acts as the routine contact point, receiving and handling requests for support (iSYS and other HR-related systems)Responds to a broad range of service requests for support by providing information to fulfill requests or enable resolution. <ul> <li>Provides first line investigation and diagnosis and promptly allocates unresolved issues as appropriate.<br>Assists with the development of standards, and applies these to track, monitor, report, resolve or escalate issues. Contributes to creation of support documentation.&nbsp;<br>&nbsp;</li> </ul> </li> </ul>';
	if ($part == 1) return $summary; else return $summary_2;
}
function advisory_sfia_get_team_user_summary($post_id, $user_id, $select_attr) {
	...
	if ($select_attr == '') {
		...
		$summary_2 = !empty($default['summary_2']) ? $default['summary_2'] : '';
	    $data .= '<form class="sfia-form single" method="post" data-meta="'.$user_id.'_pdf" data-id="'.$post_id.'">';
	        ...
	        $data .= '</div>';
	        $data .= '<div class="card">';
				$data .= '<div class="card-title-w-btn"> <h4 class="title">Summary 2</h4> </div>';
		        $data .= '<div class="card-body">';
					$data .= '<textarea name="summary_2" class="tinymce-editor">'.$summary_2.'</textarea>';
		        $data .= '</div>';
				$data .= '<div class="row mt-10">';
					$data .= '<div class="col-sm-12 text-right">';
						$data .= '<button class="btn btn-success" type="submit"><i class="fa fa-lg fa-floppy-o"></i> Save</button>';
...
function advisory_sfia_get_active_or_archive_post_id($company_id=0) {
	$postType = 'sfia';
	if (!$company_id) $company_id = advisory_get_user_company_id();
	if (advisory_metrics_in_progress($company_id, [$postType])) {
	    $form_id = advisory_get_active_forms($company_id, [$postType]);
...
function sfia_dashboard_scorecard() {
	...
	if ( !empty($type == 'missing_skills') ) echo advisory_sfiams_dashboard_scorecard();
	else {
		$post_id = !empty($_POST['post_id']) ? $_POST['post_id'] : false;
		$team_id = !empty($_POST['team_id']) ? $_POST['team_id'] : false;
			...
			foreach ($published_users as $team_id => $team_users) {
				...
				if (!empty($team_users)) {
					foreach ($team_users as $user_id => $user_name) {
						...
						// TECHNICAL SCORE
						$sfiats = advisory_get_sfiats_avg_class_and_text($head['sfiats_id']);
						
						// $html .= '<tr><td colspan="100"><br><pre>'. print_r($sfiats, true) .'</pre></td></tr>';
						...
						$html .= '<table class="table table-bordered m-0" data-toggle="collapse" data-target="#collapse_'.$user_id.'" aria-expanded="true" aria-controls="collapse_'.$user_id.'">';
							$html .= '<tbody>';
								$html .= '<tr>';
									...
									$html .= '<th class="h4 text-center p-0 '.$sfiats['cls'].'" style="border: none;width: 46px;cursor: ns-resize;">'.$sfiats['txt'].'</th>';
						...
						$html .= '<div id="collapse_'.$user_id.'" class="collapse" aria-labelledby="heading_'.$user_id.'" data-parent="#accordionSFIA">';
							$html .= '<table class="table table-bordered m-1">';
								$html .= '<tbody>';
									$html .= '<tr>';
										$html .= '<th class="t-heading-sky" style="width:auto">REQUIRED SKILL</th>';
										$html .= '<th class="t-heading-sky" style="width:59px">CODE</th>';
										$html .= '<th class="t-heading-sky" style="width:77px">TARGET</th>';
										$html .= '<th class="t-heading-sky" style="width:96px">ASSESSED</th>';
										$html .= '<th class="t-heading-sky text-center" style="width: 44px;"><span class="fa fa-info-circle" style="font-size: 19px;"></span></button></th>';
									$html .= '</tr>';
									for ($counter=0; $counter < $loop; $counter++) { 
										...
										$table = 'sections_'.$skills['category_'.$counter].'_tables_'.$skills['subcategory_'.$counter];
										$skill_name = advisory_sfia_skill_name_from_id($opts[$table], $skills['skill_'.$counter]);

										$html .= '<tr>';
											$html .= '<th class="'.$skill_class.'">'.$skill_name.'</th>';
											$html .= '<th class="'.$skill_class.'">'.$code.'</th>';
											$html .= '<th class="bg-deepgreen"> Level '.@$skills['target_level_'. $counter].'</th>';
											$html .= '<th class="'.$class.'"> '.$level_text .'</th>';
											$html .= '<th class="no-padding '.$skill_class.'"> <button class="btn btn-primary SFIAInfoBtn" type="button" title="'.$skill_name.'" data-skill="'.$infoImgLink.$code.'.pdf"><span style="line-height: 22px;" class="fa fa-info-circle"></span></button></th>';
										$html .= '</tr>';
					    			}
					    		$html .= '<tbody>';
			    			$html .= '</table>';
...
function advisory_sfia_skillgap_scorecard($post_id, $team_id, $team_name) {
	...
	if ($post_id && $team_id && $team_name) {
		...
		// $html .= '<br><pre>'. print_r($opts['sections_change_and_transformation_tables_business_change_management'], true) .'</pre>';
								...
								if (!empty($skill_array)) {
									foreach ($skill_array as $skill_id => $skill) {
										...
										if ( $score > 0) {
											...
											$scoreTxt = round($score).'%';
										} else {
											$scoreTxt = 'Missing';
											$class = advisory_get_sfia_skills_fit_class($score);
										}

										// $html .= '<tr><td colspan="4"><pre>'. print_r([$scoreTxt, $class], true) .'</pre></td></tr>';
										$html .= '<tr>';
											$html .= '<th class="'.@$skill['class'].'" style="">'.@$skill['name'].'</th>';
											...
											$html .= '<th class="'.$class.'" style="text-align:center;">'. $scoreTxt.'</th>';
										$html .= '</tr>';
									}
								}
							$html .= '</tbody>';
		...
		foreach ($team_users as $user_id => $user_name) {
			...
			if (!empty($head['role']) && !empty($skills)) {
				for ($counter=0; $counter < $loop; $counter++) { 
					...
					$code = advisory_sfia_dashboard_scorecard_get_skill_colde($opts, $skills, $counter);
					$skill_class = advisory_get_sfiar_category_class($skills['category_'. $counter]);
					//sections_change_and_transformation_tables_business_change_management
					$table = 'sections_'.$skills['category_'.$counter].'_tables_'.$skills['subcategory_'.$counter];

					$data[$head['role']][$skills['skill_'.$counter]]['name'] = advisory_sfia_skill_name_from_id($opts[$table], $skills['skill_'.$counter]);
...
function advisory_sfia_skill_name_from_id($skills, $skill_id) {
	if ( !empty($skills) ) {
		foreach ($skills as $skill) {
			if ( advisory_id_from_string($skill['name']) == $skill_id) return $skill['name'];
		}
	}
	return false;
}
function advisory_sfia_name_from_id($skills, $skill_id) {
	if ( !empty($skills) ) {
		foreach ($skills as $skill) {
			if ( advisory_id_from_string($skill['name']) == $skill_id) return $skill['name'];
		}
...
function advisory_sfia_get_permission_level($user_id=null) {
	...
	if (!empty($user_switching) && !empty($user_switching->get_old_user())) return 'full';
...
function advisory_get_sfia_pdf_data($post_id, $user_id, $preview=false) {
	$data = [];
	...
	if ($post_id && $user_id) {
		...
		if (!empty($company)) {
			...
			$data['sfia']['summary_2'] = !empty($default['summary_2']) ? $default['summary_2'] : '';

			$data['sfiats'] = advisory_get_sfiats_avg_class_and_text($head['sfiats_id']);
			
			$head['sfiats_id'] = empty($head['sfiats_id']) && $preview ? advisory_sfiats_get_active_assessments($user_id, $company->term_id) : $head['sfiats_id']; 
			$default = advisory_form_default_values(@$head['sfiats_id'], 'pdf');
			$data['sfiats']['summary'] = !empty($default['summary']) ? $default['summary'] : '';
			$data['sfiats']['summary_2'] = !empty($default['summary_2']) ? $default['summary_2'] : '';
			// $data['sfiats']['summary'] = '';
			...
			if (!empty($skills)) {
				for ($counter=0; $counter < $loop; $counter++) { 
					...
					$data['skills'][$counter]['title'] = $skill_name;
