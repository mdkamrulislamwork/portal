...
function advisory_generate_bcp_threat_questions() {
	if (is_admin() && is_edit_page()) {
		$id = @$_GET['post'];
		$meta = get_post_meta($id, 'form_opts', true);
		$data = [];
		...
		if ( !empty($meta['areas']) ) {
			foreach ( $meta['areas'] as $threat_cat ) {
				$data[] = [
					'id' => advisory_id_from_string($threat_cat['name']) . '_questions',
					'type' => 'group',
					'title' => $threat_cat['name'],
					'desc' => 'Each Question should be unique',
					'button_title' => 'Add New',
...
function bcpInputController()
{
    ...
    $specialUser    = get_the_author_meta( 'specialriskuser', $userId );
